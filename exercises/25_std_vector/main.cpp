...
int main(int argc, char **argv) {
    {
        ...
        ASSERT(vec.size() == 5, "Fill in the correct value.");//size() 返回 vector 中元素的个数
        // THINK: `std::vector` 的大小是什么意思？与什么有关？
        ASSERT(sizeof(vec) == 24, "Fill in the correct value.");//sizeof(vec) 返回 vector 对象本身的大小，而不是它包含的元素的大小.std::vector包含 3 个指针（data、size、capacity）
        // THINK: `std::vector` 的数据存储在哪里？如何访问？

        // data() 返回指向 vector 中第一个元素的指针
        // 注意：std::vector<int> 的元素是 int 类型，每个 int 占用 4 字节
        // 因此，5 个 int 的总大小为 5 * 4 = 20 字节
        // 但是，std::vector 对象本身还包含一些额外的信息（如大小和容量），
        // 所以 sizeof(vec) 返回的是 24 字节（具体大小可能因编译器和平台而异）。
        // 这就是为什么我们需要使用 std::memcmp 来比较内容，而不是直接比较 sizeof(vec)。
        // std::memcmp 用于比较两个内存块的内容
        // 这里我们将 vec.data() 的内容与 ans 数组进行比较
        int ans[]{1, 2, 3, 4, 5};
    ...
    }
    {
        ...
        }
        {
            ...
            vec.clear();// 清空 vector 中的所有元素size=0,但不释放内存
