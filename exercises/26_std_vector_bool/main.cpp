...
//bool类型是1byte,但vector<bool>会进行位压缩，每个bool只占1bit
//根据 C++ 标准，若容器 C<T>支持 operator[]，则表达式 T* p = &c[0]必须合法。但 vector<bool>不满足此条件，因此严格来说它并非标准容器
...
int main(int argc, char **argv) {
    ...
    // NOTICE: 平台相关！注意 CI:Ubuntu 上的值。CI是持续集成环境
    std::cout << "sizeof(std::vector<bool>) = " << sizeof(std::vector<bool>) << std::endl;
    ASSERT(sizeof(vec) == 40, "Fill in the correct value.");//gcc是40
    {
        vec[20] = false;
